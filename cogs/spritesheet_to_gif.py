...
class SpritesheetToGIF(commands.Cog):
    def __init__(self, bot: commands.Bot):
        self.bot = bot

    @app_commands.command(
        name="spritesheet_to_gif",
        description="Extract frames from a spritesheet and generate a GIF"
    )
    ...
    @app_commands.describe(
        ...
        scale="Upscale factor for exported GIF (keeps pixels sharp, default 2)"
    )
    async def spritesheet_to_gif(
        self,
        ...
        try:
            ...
            if cols is None and rows is None:
                # Derive from image size and frame dimensions
                ...
                # rows given, cols = total frames / rows (rounded down)
            ...
            if scale != 1:
                ...
                for f in frames:
                    # Use NEAREST neighbour for pixel‑perfect scaling
                    scaled = f.resize(new_size, Image.NEAREST)
                    scaled_frames.append(scaled)
                frames = scaled_frames
                frame_width, frame_height = new_size  # update for response
            ...
            frames[0].save(
                ...
                palette=None  # let Pillow choose a palette
            )
            output.seek(0)

            # Send result
            file = discord.File(output, filename="animation.gif")
            await interaction.followup.send(
                content=(
                    f"✅ GIF generated:\n"
                    f"• Frames: {len(frames)}\n"
...
async def setup(bot: commands.Bot):
    await bot.add_cog(SpritesheetToGIF(bot))